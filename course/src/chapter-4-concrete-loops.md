...
# Chapter 4: Build Variable-Width Rows Transactionally

Chapter 3 can lift a scalar function when one owned value is enough to represent an output row.
A string row is different: its UTF-8 bytes, terminal offset, and validity bit must become visible
together. This chapter makes that publication boundary explicit.

Start from your completed Chapter 3 workspace, copy the cumulative tests, and run the focused test
once before editing:
