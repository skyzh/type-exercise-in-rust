...
static-sitemap-cli -b https://skyzh.github.io/type-exercise-in-rust -r book -f xml -o \
  | sed '/<lastmod>/d' > "$generated_dir/sitemap.xml"
static-sitemap-cli -b https://skyzh.github.io/type-exercise-in-rust -r book -f txt -o \
  > "$generated_dir/sitemap.txt"
...
if [[ "$mode" == "--check" ]]; then
  ...
  if [[ "$stale" -ne 0 ]]; then
    echo "sitemap is stale; run course/sitemap.sh and commit the result" >&2
    exit 1
  fi
else
