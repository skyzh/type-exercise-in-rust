...
fn keeps_one_checked_decimal_descriptor_per_array() {
    ...
    assert!(builder.try_push(Some(Decimal::try_new(7, other).unwrap())).is_err());
    assert_eq!(builder.len(), 1, "a rejected row must not mutate the builder");
}
...
fn erases_and_recovers_dense_values_with_checked_types() {
    ...
    assert!(F32Array::from_slice(&[Some(f32::NAN)]).get(0).unwrap().is_nan());
    assert_eq!(
