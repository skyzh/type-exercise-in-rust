use crate::{
    Array, ArrayImpl, ColumnViewImpl, DataType, I32Array, ListArray, ListError, ListScalar,
    PhysicalType, ScalarImpl, ScalarRefImpl, StringArray, TypeMismatch,
};
...
fn raw_parts_reject_bad_types_offsets_and_null_spans() {
    ...
    assert_eq!(
        ListArray::try_from_raw_parts(PhysicalType::String, values.clone(), vec![0, 2], vec![true],),
    ...
    assert_eq!(
        ...
        Err(ListError::OffsetOutOfOrder {
            row: 1,
            start: 2,
            end: 1,
        })
    ...
    assert_eq!(
        ListArray::try_from_raw_parts(PhysicalType::Int32, values.clone(), vec![0, 2], vec![false],),
        Err(ListError::NullRowHasValues {
            row: 0,
            start: 0,
            end: 2,
        })
...
fn owned_and_borrowed_lists_slice_without_leaking_other_items() {
    ...
    assert_eq!(
        value.as_list_ref().slice(2, 4),
        Err(ListError::RangeOutOfBounds {
            start: 2,
            end: 4,
            len: 3,
        })
...
#[test]
fn erased_list_columns_cover_array_constant_null_and_dictionary() {
    ...
    assert_eq!(
        array_view.get(3),
        Err(ListError::RowOutOfBounds { row: 3, len: 3 })
    );
    ...
    let dictionary = ColumnViewImpl::dictionary(&keys, &array)
        .unwrap()
        .try_as_list(PhysicalType::Int32)
        .unwrap();
    assert_eq!(dictionary.get(0).unwrap().unwrap().len(), 0);
    assert_eq!(dictionary.get(1).unwrap(), None);
    assert_eq!(dictionary.get(2).unwrap().unwrap().len(), 2);
...
fn nested_list_construction_is_rejected_explicitly() {
    ...
    assert_eq!(
        ListScalar::try_new(child.clone().into()),
