...
#[cfg(test)]
mod tests {
    use expr_common::array::{Array, F64Array, I16Array, I32Array, PhysicalType, StringArray};
    ...
    #[test]
    fn registry_rejects_unknown_function() {
        let registry = FunctionRegistry::with_builtins();
        assert_eq!(
            registry
                .bind_binary("missing", DataType::Integer, DataType::Integer)
                .err()
                .unwrap(),
            BindError::UnknownFunction("missing".to_owned())
        );
    }

    #[test]
    fn bound_expression_rejects_wrong_physical_input() {
        let registry = FunctionRegistry::with_builtins();
        let expression = registry
            .bind_binary("contains", DataType::Varchar, DataType::Varchar)
            .unwrap();
        let wrong = I32Array::from_slice(&[Some(1)]).into();
        let string = StringArray::from_slice(&[Some("1")]).into();
        let error = expression
            .eval(&[
                ColumnViewImpl::array(&wrong),
                ColumnViewImpl::array(&string),
            ])
            .unwrap_err();
        assert_eq!(
            error.downcast_ref::<BindError>(),
            Some(&BindError::PhysicalTypeMismatch {
                index: 0,
