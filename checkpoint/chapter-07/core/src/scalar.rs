...
/// An owned scalar connected to its borrowed value and dense array.
pub trait Scalar:
...
/// A copyable scalar borrowed from an owned value or dense array.
...
macro_rules! physical_type_of_scalar {
    ...
    ($( { $kind:ident, $variant:ident, $array:ident, $builder:ident, $owned:ty, $borrowed:ty } ),+ $(,)?) => {
        ...
        impl ScalarImpl {
            pub fn physical_type(&self) -> PhysicalType {
                match self {
                    $(Self::$variant(value) => physical_type_of_scalar!($kind, $variant, value)),+
                }
            }
        }
        ...
        impl ScalarRefImpl<'_> {
            ...
            pub fn to_owned_scalar(self) -> ScalarImpl {
                match self {
                    $(Self::$variant(value) => own_scalar_ref!($kind, $variant, value)),+
                }
            }
        }
