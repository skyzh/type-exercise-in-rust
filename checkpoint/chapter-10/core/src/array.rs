...
macro_rules! physical_type_of_array {
...
/// A nullable, dense collection of one physical value family.
pub trait Array:
...
/// The append-only companion used to construct one dense array.
    ...
    ($( { $kind:ident, $variant:ident, $array:ident, $builder:ident, $owned:ty, $borrowed:ty } ),+ $(,)?) => {
        ...
        impl ArrayImpl {
            pub fn physical_type(&self) -> PhysicalType {
                match self {
                    $(Self::$variant(array) => physical_type_of_array!($kind, $variant, array)),+,
            ...
            pub(crate) fn slice(&self, start: usize, end: usize) -> Result<Self, ListError> {
                ...
                match self {
                    $(Self::$variant(array) => array_slice!($kind, $variant, $array, array, start, end)),+,
                    ...
                    PhysicalType::Decimal(decimal_type) => {
                        let typed = values
                            ...
                            .map(|value| match value {
                                ...
                                Some(ScalarImpl::Decimal(value)) => Err(anyhow!(
                        ...
                        Ok(Self::Decimal(DecimalArray::try_from_slice(
                            *decimal_type,
                            &typed,
                        )?))
                    }
...
macro_rules! define_array_family {
    (copy, $variant:ident, $array:ident) => {
        define_infallible_array_family!($variant, $array);
    };
    (borrowed, $variant:ident, $array:ident) => {
        define_infallible_array_family!($variant, $array);
    };
...
macro_rules! define_infallible_array_family {
