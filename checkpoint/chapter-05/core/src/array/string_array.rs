...
/// An unpublished output row. Consuming [`Writer::write`] publishes exactly one row.
pub struct Writer<'a> {
...
impl StringValueWriter<'_> {
    /// Append one UTF-8 fragment to the pending value.
...
impl StringArrayBuilder {
    ...
    /// Build one non-null value in place, publishing row metadata only on success.
    pub fn try_push_with<E>(
        &mut self,
        write: impl FnOnce(&mut StringValueWriter<'_>) -> Result<(), E>,
    ) -> Result<(), E> {
        let start = self.data.len();
        match write(&mut StringValueWriter {
            data: &mut self.data,
        }) {
            Ok(()) => {
