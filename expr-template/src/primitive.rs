...
mod tests {
    ...
    #[test]
    fn falls_back_for_dictionary_and_null_constant() {
        let values: ArrayImpl = I32Array::from_slice(&[Some(1), Some(2)]).into();
        let indices = [Some(0), None, Some(1)];
        let dictionary = ColumnViewImpl::dictionary(&indices, &values).unwrap();
        let expression =
            PrimitiveBinaryExpression::<i32, i32, i32, _>::new(|left, right| left + right);
        ...
        assert_eq!(result.get(0), Some(ScalarRefImpl::Int32(11)));
        assert_eq!(result.get(1), None);
        assert_eq!(result.get(2), Some(ScalarRefImpl::Int32(12)));

        let left: ArrayImpl = I32Array::from_slice(&[Some(1), Some(2)]).into();
        let result = expression
            .eval_views(
                ColumnViewImpl::array(&left),
                ColumnViewImpl::null(expr_common::array::PhysicalType::Int32, 2),
            )
            .unwrap();
